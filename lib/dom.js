            ...
            if (item) {
              ...
              page.open(url.data(), function(err, status) {
                if (status !== 'success') {
                  ...
                  if (count < connect_limit) {
                    setTimeout(function() {
                      console.error("\trecur " +  url.data() + count);
                      repeater(page, item, count + 1);
                    }, 100*count > 500 ? 500 : 100*count);
                  } else {
...
var dom;
dom = function dom(domain, action, forceEngine) {
  ...
  switch (browser) {
    case 'jsdom':
      return createJsdom(worker, includejsfiles, worker.encoding || script.encoding || 'utf8');
    case 'phantom':
