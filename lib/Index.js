    ...
    while (index.getType() !== null) {
      if (index.getType() === 'leaf') {
        list.push('<navPoint id="html' + count + '" playOrder="' + count +
                 ...
                 '<content src="' + 'OEBPS/'+ (new Url(index.getUrl())).getFileName() +
                 '"/></navPoint>');
        count += 1;
      } else if (index.getType() === 'branch') {
        index.enterThisBranch();
        section_count += 1;
        list.push('<navPoint id="chapter' + (cCount++) + '" playOrder="' + (count++) +
                   '"><navLabel><text>' + index.getLabel() + '</text></navLabel>' +
                   '<content src="catalog.html#section' + section_count + '"/>');
        list.push(compose());
        list.push('</navPoint>');
  ...
  function compose() {
    ...
    while (index.getType() !== null) {
      if (index.getType() === 'leaf') {
        ...
        index.enterThisBranch();
        section_count+=1;
        con.push('<li><a name="section' + section_count + '">' + index.getLabel() + '</a>');
        con.push('<ul>');
        con.push(compose());
        con.push('</ul></li>');
        index.leaveThisBranch();
