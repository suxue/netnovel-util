...
function package_content_opf(index, zip, contents) {
  ...
  contents.forEach(function(item) {
    if (item[2]) {
      var filename = item[1];
      zip.includeLocalFile(filename, 'OEBPS/'+filename);
    }
  });
...
function package_ncx(index, zip) {
  ...
  var section_count = 0;
  var article_count = 0;
  var sections = [{list:[], title: index.title(), filename:'OEBPS/section0.html'}];
  function compose(parent_section) {
    ...
    while (index.getType() !== null) {
      if (index.getType() === 'leaf') {
        article_count += 1;
        filename = (new Url(index.getUrl())).getFileName();
        contents.push(["html" + article_count, filename, true]);
        sections[parent_section].list.push([index.getName(), filename]);
        list.push('<navPoint id="html' + count + '" playOrder="' + count +
                   '"><navLabel><text>' + index.getName() + '</text></navLabel>' +
                 '<content src="OEBPS/' + filename + '"/></navPoint>');
        count += 1;
        ...
        contents.push(["section" + section_count, filename]);
...
function package_tocpage(index, zip) {
  ...
  function compose() {
    ...
    while (index.getType() !== null) {
      if (index.getType() === 'leaf') {
        filename = "OEBPS/" + (new Url(index.getUrl())).getFileName();
