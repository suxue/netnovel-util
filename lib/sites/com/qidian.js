...
function coverpage_indexer(window, bookid, index) {
...
function tocpage_indexer(window, bookid, index) {
...
exports.indexer = function(window, url, index) {
  Url = url.constructor;
  var rc = coverpage_pattern.exec(url.data());
  var bookid;
  if (rc) {
    bookid = rc[1];
    return coverpage_indexer(window, bookid, index);
  }
  rc = tocpage_pattern.exec(url.data());
  if (rc) {
    bookid = rc[1];
    return tocpage_indexer(window, bookid, index);
  }
